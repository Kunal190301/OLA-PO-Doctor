...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (versionEl) {
    versionEl.textContent = '3.6.0 (Latest)';
  }
  ...
  reader.onload = function(e) {
    try {
      ...
      if (detectedKey === 'ola') {
        ...
        // Diagnostic logs to check raw values
        console.log("=== OLA FILE DIAGNOSTICS ===");
        console.log("Total rows parsed:", cleanedData.length);
        const accountsSeen = [...new Set(cleanedData.map(r => r.account))];
        console.log("Accounts present in OLA:", accountsSeen);
        accountsSeen.forEach(acc => {
          const accRows = cleanedData.filter(r => r.account === acc);
          const sampleRaw = accRows.slice(0, 5).map(r => r['Average OLA']);
          console.log(`Account "${acc}" has ${accRows.length} rows. Sample 'Average OLA' raw values:`, sampleRaw);
        });
        console.log("============================");
...
function updateWeeksTriggerText() {
  ...
  } else if (state.olaData.length > 0 && state.allocationData.length > 0 && state.selectedChains.length > 0) {
    if (btnProcess) btnProcess.removeAttribute('disabled');
  }
}
